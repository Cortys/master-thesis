...
As per our definition of \ac{lta} from \cref{sec:ltag:definition}, the quality of an \ac{lta} formulation is determined by its chosen solution to the \ac{ltd} problem.
...
In 2-\acs{wl} on the other hand, the neighbors of $\textcolor{t_blue}{e_{ij}}$ are the edge pairs ${\left\{(\textcolor{t_red}{e_{il}}, \textcolor{t_darkgreen}{e_{lj}})\right\}}_{v_l \in \mathcal{V}_G}$, i.e.\ all possible walks of length two that start at $v_i$ and end at $v_j$. % chktex 21 chktex 25
...
As described in \cref{sec:related:character:wl}, the discriminative power of a model is, by itself, not necessarily relevant for real-world \ac{gcr} problems.
...
\begin{enumerate*}[label={\circled{\small\arabic*}}]
	...
	\item Then the discriminative and computational power of this operator is analyzed.
...
\end{enumerate}
Based on the 2-multiset and the neighborhood localization simplifications, we now define the 2-\ac{wl} convolution operator and the corresponding so-called \textit{2-\acs{wl}-\acs{gnn}}.
...
In this section we will now analyze the power of \acp{gnn} which use the 2-\acs{wl} convolution operator that was just defined.
...
\begin{proof}
	...
	\begin{gather*}\allowdisplaybreaks
		\begin{aligned}
			...
			&= \begin{cases}
				\textcolor{t_blue}{(1, 0, w_{ii}, 0)} + \textcolor{t_darkgreen}{\left(0, 0, 0, 2\textcolor{t_red}{w_{ii}} Z^{(t-1)}[v_i]\right)} + \smashoperator[lr]{\sum_{v_l \in \Gamma_{G^1}(v_i)}} \textcolor{t_darkgreen}{\left(0, \tfrac{2}{\textcolor{t_red}{2}} \textcolor{t_red}{Z^{(t-1)}[v_i]} w_{il}, 0, 0\right)} & \text{if $i = j$} \\ % chktex 25
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=\linewidth]{gfx/ltd/wl2-vert-agg-intuition.pdf}
		\caption[Intuition for how $S^{(t, 1)}$ and $S^{(t, 2)}$ compute vertex neighborhood sums.]{
			Intuition for how $S^{(t, 1)}$ and $S^{(t, 2)}$ compute vertex neighborhood sums in two steps.
			For simplicity, weights are ignored, i.e.\ all $w_{ij} = 1$.
			For each self-loop/edge $e_{ij} \in \mathcal{E}_{G^1}$, the set of localized 2-\acs{wl} neighbors $\Gamma_{G^1}(v_i) \cap \Gamma_{G^1}(v_j)$ is shown in the middle, after the first step.
			Note that the colors in this illustration are unrelated to the colored parts in the previous equations.
		}\label{fig:ltd:wl2-vert-agg-intuition}
	\end{figure}
