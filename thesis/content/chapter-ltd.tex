...
In this section we describe a convolution operator which uses edge tuple neighbors just like 2-\acs{wl} to overcome the limitations of 2-\acsp{gnn}.
...
Similar to the 2-LWL kernel described in \cref{sec:related:gcr:kernel}, the 2-\acs{wl} convolution operator reduces the computational cost of 2-\acs{wl} via two simplifications:
\begin{enumerate}[label=\textbf{\arabic*.}]
	...
	\item \textbf{Neighborhood localization:}
		2-\acs{wl} refines the color of all tuples $(v_i, v_j) \in \mathcal{V}_G^2$ by hashing its current color and the colors of all pairs ${\{ ((v_i, v_l), (v_l, v_j)) \}}_{v_l \in \mathcal{V}_G}$.
		...
		We address this issue by reducing both the number of colored tuples as well as the number of neighbors of each tuple.
		Through this reduction the time complexity of a refinement step becomes $\mathcal{O}(n d^k)$ where $d \coloneqq \max_{v \in \mathcal{V}_G} \left|\Gamma_G(v)\right|$ is the maximum vertex degree and $k \in \mathbb{N}$ is the freely choosable so-called \textit{neighborhood degree}.
		We will now look at the details of this simplification.
\end{enumerate}
\begin{defn}
	The \textit{$k$-th power of a graph $G$} is defined as
	\begin{align*}
		G^k \coloneqq \left(\mathcal{V}_G, \left\{ e_{ij} \in \mathcal{V}_G^2\, |\, d_{\mathrm{SP}, G}(v_i, v_j) \leq k \right\}\right) % chktex 21
	\end{align*}
	...
	Note that $G^1$ does not generally equal $G$ because $G^1$ has self-loop edges $(v_i, v_i) \in \mathcal{E}_{G^1}$ for all vertices.
\end{defn}
\begin{defn}
	The \textit{initial feature matrix $Z_G^{(0)}$} of the 2-\acs{wl} convolution operator with the neighborhood degree $k \in \mathbb{N}$ combines the vertex features $x[v_i] \in \mathcal{X}_{\mathcal{V}} = \mathbb{R}^{d_{\mathcal{V}}}$ and the edge features $x[e_{ij}] \in \mathcal{X}_{\mathcal{E}} = \mathbb{R}^{d_{\mathcal{E}}}$ of a given graph $G$.
	More specifically $Z_G^{(0)} \in \mathbb{R}^{{|\mathcal{E}_{G^k}|} \times (d_{\mathcal{V}} + d_{\mathcal{E}})}$ assigns a row vector $Z_G^{(0)}[e_{ij}]$ to all edges $e_{ij} \in \mathcal{E}_{G^k}$ of $G^k$.
	Those edge feature vectors are defined by the following vector concatenation:
	\begin{align*}
	...
	\end{align*}
\end{defn}
Using this definition, the number of convolved feature vectors can be controlled via the neighborhood degree $k$.
For $k = 1$ only the vertices and edges of a given graph $G$ have a feature vector.
For $k > 1$ nonexistent edges $e_{ij} \notin \mathcal{E}_G$ also get feature vectors.
\begin{defn}
	We define the \textit{2-\acs{wl} graph convolution operator} with the neighborhood degree $k \in \mathbb{N}$ as
	\begin{gather*}
		Z_G^{(t)}[e_{ij}] \coloneqq \sigma\left( Z_G^{(t-1)}[e_{ij}] W_{\mathrm{L}}^{(t)} + \smashoperator[lr]{\sum_{v_l \in {\Gamma_{G^k}(v_i) \cap \Gamma_{G^k}(v_j)}}} \kappa^{(t)}\left( Z_G^{(t-1)}[e_{ij}], \ldblbrace Z_G^{(t-1)}[e_{il}], Z_G^{(t-1)}[e_{lj}] \rdblbrace \right) \right) \\
		...
		\text{.}
	\end{gather*}
	The operator is parameterized by the three matrices $W_{\mathrm{L}}^{(t)}, W_{\mathrm{F}}^{(t)},  W_{\Gamma}^{(t)} \in \mathbb{R}^{d^{(t-1)} \times d^{(t)}}$.
\end{defn}
