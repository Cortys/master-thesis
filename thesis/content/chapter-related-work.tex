...
\begin{figure}[h]
	...
	\caption[Two simple non-isomorphic graphs that are indistinguishable via 1-\acs{wl}.]{
		Two simple non-isomorphic graphs that are indistinguishable via 1-\acs{wl}.
...
Analogous to the 1-dimensional case, the $k$-dimensional color refinement step is defined by
\begin{align}
	\chi_{G,k}^{i+1}(v) := &\ h\left(\chi_{G,k}^{i}(v), \ldblbrace (\chi_{G,k}^{i}(v[u/1]), \dots, \chi_{G,k}^{i}(v[u/k]))\, |\, u \in \mathcal{V}_G \rdblbrace\right)\label{eq:related:wl1-refine}\\
...
In $k$-\acs{wl} the color of a vertex $k$-tuple $v$ is refined by combining the colors of its neighborhood which is defined as the set of all $k$-tuples in which at most one vertex differs from $v$.
