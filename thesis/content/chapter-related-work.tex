...
\end{defn}
\citet{Alzaga2010} have shown that the cospectrality test can be more powerful than 1-\acs{wl} but is always weaker than 2-\acs{wl}, i.e.\ $G \mathrel{\simeq_2} H \implies G \mathrel{\simeq_\lambda} H$.
Despite this limit on the discriminative power of the graph spectrum, it is still useful for determining graph similarity, e.g.\ by defining a distance measure on the vector space of spectra~\cite{Gu2015}.
...
Consequently a small graph $G$ will always have a large spectral distance to a large graph $H$ (assuming $|\mathcal{V}_G| \ll |\mathcal{V}_H|$) irrespective of their similarity when ignoring the scale difference.
...
\end{defn}
The eigenvalues $\lambda^{\text{sym}}_{G, i}$  of this Laplacian all lie in the range $[0, 1]$.
There are also other ways to normalize the spectrum, e.g.\ by using the random walk Laplacian $L^{\text{rw}}_G \coloneqq D^{-1} L_G$, which we will however not cover here (see \citet{Shuman2013}).
This concludes our overview of spectral graph characterization.
