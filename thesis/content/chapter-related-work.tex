...
\begin{defn}
	$k$-\acs{wl} computes a function $f: \mathcal{G} \to P$ iff.\ that function can be expressed as $f(G) = g(\chi_{G, k}^{*})$ via some function $g: (\mathcal{V}_G^k \to \mathcal{C}) \to P$.
\end{defn}
\begin{defn}
...
this counterexample can be trivially generalized to all larger $m$ by replacing the ``six-cycle'' graph with a ``$(2m)$-cycle-graph'' graph and the ``two-triangles'' graph with a ``two-$m$-cycles'' graph, preserving the 2-regularity and therefore the 1-\acs{wl} indistinguishability.
