...
The spectral perspective on \ac{gcr} expects a graph $G$ with real vertex feature vectors $x_G[v_i] \in \mathbb{R}^f$ as its input.
We call $X_G \coloneqq {(x_G[v_1], \dots, x_G[v_n])}^{\top} \in \mathbb{n \times f}$ the vertex feature matrix of $G$.
Note that each of the $f$ columns of $X_G$ can be interpreted as a separate graph signal function $x_{G,j}: \mathcal{V}_G \to \mathbb{R}$.
...
Via the convolution theorem $\widehat{g * x} = \hat{g} \cdot \hat{f}$ we can however still define a graph convolution operator that works directly in the spectral domain:
\begin{align}
	g * x_{G,j} \coloneqq U \hat{g}(\Lambda) U^{\top} x_{G,j}
	\text{ with the eigendecomposition $L_G = U \Lambda U^{\top}$}
\end{align}
To see the connection to the convolutional theorem, note that the matrix of eigenvector rows $U^{\top}$ describes the graph Fourier transform and conversely that $U$ describes the inverse graph Fourier transform.
\citet{Bruna2013} proposed a \ac{nn} architecture that learns such a spectral filter $\hat{g}$.
