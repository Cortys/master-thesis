...
\begin{alignat*}{2}
	\text{\ac*{gcn}:}\quad&& Z^{(t)}[v] &= \sigma\left(\left( {\mu(v)}^2 \textcolor{t_blue}{Z^{(t-1)}[v]} + \smashoperator[lr]{\sum_{u \in \Gamma_G(v)}} \mu(v) \mu(u) \textcolor{t_red}{Z^{(t-1)}[u]} \right) \Theta^{(t)} \right) \\ % chktex 25
...
\begin{prop}[see {\citet[lemma 2]{Xu2018}} for the proof]\label{prop:related:gcnn-wl1-limit}
	The discriminative power of a \ac{gcnn} that convolves vertex feature vectors via a convolution operator of the form $Z^{(t)}[v] = h^{(t)}\left(Z^{(t-1)}[v], \ldblbrace Z^{(t-1)}[u]\, |\, u \in \Gamma_G(v) \rdblbrace\right)$ is upper-bounded by that of 1-\acs{wl}, where $h^{(t)}: {(\mathbb{R}^{d^{(t-1)}})}^{*} \to \mathbb{R}^{d^{(t)}}$ is an arbitrary vertex neighborhood hashing function.
