...
Before combining \ac{lta} and \ac{gcr} as described in \cref{sec:intro:questions}, we first give an overview of the state-of-the-art in both fields of research.
...
\begin{enumerate}
	...
	\item Then we look at the domain of structured inputs.
		...
		We will look at three approaches for graph characterization: The strict graph isomorphism test, the so-called \acl{wl} coloring and lastly the notion of graph spectra.
...
\begin{enumerate}[label=\textbf{\arabic*.}]
	\item \textbf{Aggregation:}
		A variadic aggregation function $\mathcal{A}: \mathcal{Y}^{*} \to \mathcal{Y}$ that estimates composition scores has to be learned, i.e.\@ $y \approx \hat{y} = \mathcal{A}(y_{1}, \dots, y_{n})$.
	...
	\item \textbf{Disaggregation:}
		...
		To learn this derivation function $f: \mathcal{X} \to \mathcal{Y}$, only the constituent vectors ${\ldblbrace x_i \rdblbrace}_{i = 1}^{n}$ and the composition score $y$ is given.
...
There the basic idea is to express composition scores as fuzzy truth assignments $y \in [0, 1]$.
Such a score assignment $y$ is modeled as the result of a parameterized logical expression of constituent assignments $y_i \in [0, 1]$.
...
Overall, such an \ac{lta} model is therefore described by the uninorm parameter $\lambda$ and the regression coefficients $\theta$.
...
Instead, they are interpolated using a so-called \ac{bum} function $q: [0, 1] \to [0, 1]$ which takes constituent positions that are normalized to the unit interval, i.e.\ $\frac{i}{n} \in [0, 1]$.
The \ac{bum} function $q$ is used to interpolate a weight for any normalized sort position via $\lambda_i \coloneqq q\left(\frac{i}{n}\right) - q\left(\frac{i - 1}{n}\right)$.
...
\end{figure}
In the \ac{owa} \ac{lta} approach the \ac{bum} function $q$ is modeled as a piecewise linear spline.
...
For all practical purposes, \ac{gi} can therefore be solved efficiently; e.g.\ via the \texttt{nauty} program~\cite{McKay2013}\cite{McKay}.
...
Analogous to the 1-dimensional variant in \cref{defn:related:wl1-refine}, the $k$-dimensional color refinement step is defined by
...
Since the time complexity of $k$-\acs{wl} grows exponentially with $k$~\cite[cor.~1.9.7]{Immerman1990}, it does not provide an efficient universal solution to the \ac{gi} problem.
...
\begin{defn}
	...
	\end{aligned}$
\end{defn}
Generally speaking, this restriction of 2-\acs{wl} is not an issue since strongly regular graphs do not appear in typical \ac{gcr} datasets.
...
Similarly the inverse \ac{ft} can be expressed as $\mathcal{F}_G^{-1} = U_G^{-1} = U_G^{\top}$ because $U_G$ is a real orthogonal matrix.
...
Fingerprint embeddings are for example used in multiple state-of-the-art toxicity prediction tools like RASAR~\cite{Luechtefeld2018}\cite{ToxTrack}, the \ac{toxTest}~\cite{TEST} or ProTox~\cite{Drwal2014}\cite{Banerjee2018}\cite{ProTox}.
...
Those probabilities are described by the overlaps of the embedding $\varphi(w_i)$ of a word $w_i$ and the embeddings $\varphi_{\Gamma}(w_j)$ of its context words $w_j$.
...
\texttt{graph2vec} gets a set of graphs $\mathcal{G}_{\mathcal{D}} = \{ G_1, \dots G_N \}$ as input and outputs graph embeddings $\varphi(G_1), \dots, \varphi(G_N)$.
While in \texttt{word2vec} every word can be a context root as well as a context element, \texttt{graph2vec} uses the graphs $\mathcal{G}_{\mathcal{D}}$ as context roots.
...
\end{equation}
This context describes a graph as the set of its 1-\acs{wl} colors that occur within the first $T$ refinement steps.
Since \ac{wl} colors do not incorporate continuous feature vectors, \texttt{graph2vec} can only be applied to graphs with discrete vertex labels.
Using the previous definitions, the context root embedding function has the signature $\varphi: \mathcal{G} \to \mathbb{R}^d$, while the context element embedding function is of type $\mathcal{C} \to \mathbb{R}^d$.
To find those embeddings, the \texttt{word2vec} objective from \cref{eq:related:word2vec} is reused.
Analogous to \texttt{word2vec}, \texttt{graph2vec} therefore embeds graphs that share the same \ac{wl} colors close to each other, whereas graphs that do not share colors tend to be embedded further away from each other.
...
even for $k = 2$ the cost can become infeasibly high when working with large graphs.
...
\citet{Morris2017} have empirically shown that focusing on local graph structures via LWL often actually results in a better predictive performance than that achieved by the computationally more expensive GWL kernel.

\subsection{Graph Neural Networks}%
\label{sec:related:gcr:nn}

The last family of \ac{gcr} approaches we will look at is that of \acfp{gnn}.
...
Then the continuous analogue to the injective 1-\acs{wl} hash function $h: \mathcal{C}^{*} \to \mathcal{C}$ can be defined as $h_{\mathrm{\acs*{gcn}}}(z_0, z_1, \dots, z_m) = \sigma(\sum_{j=0}^m \mu(v_0) \mu(v_j) z_j \Theta^{(t)})$ which computes a single color vector from multiple color vectors $z_j$ with the corresponding vertices $v_j$.
...
\begin{prop}[see {\citet[lemma 2 and theorem 3]{Xu2018}} for the proof]\label{prop:related:gcnn-wl1-limit}
...
As expected, \ac{gin} therefore fits training data better than less powerful, non-injective \acp{gcnn} like \ac{gcn}.
Interestingly, \acp{gin} additionally seem to generalize better on test data than non-injective methods~\cite{Xu2018}\cite{Errica2020}.
It is not yet fully understood why this is the case.

\paragraph{Higher dimensional \ac{wl} \acp{gnn}}
The majority of \ac{gnn} methods uses a vertex neighborhood convolution scheme which limits their discriminative and, more importantly, computational power to that of 1-\acs{wl} (see \cfullref{defn:related:wl-compute}).
...
This can be seen by realizing that a stack of $k$-\acs{gnn} convolution layers, with $W_{\Gamma}^{(t)} = \mathbf{0}$ for all layers, simulates a \ac{mlp}.
...
\begin{enumerate}[label=\textbf{\arabic*.}]
	\item \textbf{Hierarchical Pooling:}
		This type of pooling is similar to that found in \acp{cnn} which iteratively reduce the resolution of images.
		Similarl,y hierarchical graph pooling iteratively \textit{coarsens} a graph every couple of convolutional layers.
...
The set of convolved feature vectors is then filtered down from the size $m$ to $p \coloneqq \lceil r m \rceil$, with $r \in [0, 1]$, by removing the vectors of the vertices with the lowest attention scores.
