...
\begin{defn}
	...
	If $\mathcal{X}_{\mathcal{E}} = \mathbb{R}$, $x_G[e_{ij}]$ can for example be interpreted as an edge weight of $e_{ij}$.
\end{defn}
...
\begin{defn}
	Two encoded graphs $G$ and $H$ with vertices $\mathcal{V}_G = \{ v_1, \dots, v_n \}$ and $\mathcal{V}_H = \{ w_1, \dots, w_n \}$ respectively are \textit{isomorphic} ($G \simeq H$) iff.\
\end{defn}
...
\end{defn}
As we will see, the \ac{wl} color refinement does not depend on vertex or edge encoding orderings, thus any difference in the final coloring of two graphs always implies the non-isomorphism of the colored graphs, i.e.\ $G \mathrel{{\not\simeq}_k} H \implies G \not\simeq H$.
