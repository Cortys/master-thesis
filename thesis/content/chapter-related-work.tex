...
The disaggregation problem is tackled by adding the scores $(y_1, \dots, y_M) \in \mathbb{R}^M$ to the learnable parameters of the model where $M$ is assumed to be the finite number of constituents.
Currently the \ac{owa} approach requires all possible constituents to be part of the training dataset since it does not consider constituent features $x_i$ to predict the scores of unseen constituents.
