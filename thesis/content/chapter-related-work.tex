...
\end{align*}
Here $c(v) \coloneqq {\left(\left| \Gamma_G(v) \right| + 1\right)}^{-\frac{1}{2}}$ are normalization factors introduced by $\tilde{D}_G^{-\frac{1}{2}}$ which can be ignored for now.
The comparison shows that \ac{gcn} convolutions can be understood as iterative refinements of continuous ``color vectors'' $Z^{(i)}[v] \in \mathcal{R}^{d^{(i)}}$.
The continuous analogue to the injective 1-\acs{wl} hash function $h: \mathcal{C}^{*} \to \mathcal{C}$ is the function $h_{\text{\acs{gcn}}}(z_1, \dots, z_m) = \sigma(\sum z_i \Theta^{(i)})$.

The relation between this type of spatial convolution and 1-\acs{wl} was formally analyzed by \citet{Xu2018}.
They show that the discriminative power of \acp{gnn} that use a vertex-aggregation scheme is in fact bounded by 1-\acs{wl}.
Moreover they show that the \ac{gcn} architecture is strictly less powerful than 1-\acs{wl} due to the fact that $h_{\text{\acs{gcn}}}$ is not an injective hash function.
