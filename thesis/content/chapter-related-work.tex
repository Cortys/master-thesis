...
Then we will look at the current approaches to tackle the \ac{gcr} problem.
...
The basic idea of graph embedding approaches is to map a graph $G$ to some finite vector space $\mathbb{R}^d$.
By embedding a graph any classification or regression algorithm that works with vectors can then be applied.
There are three main types of graph embedding approaches.

\paragraph{Fingerprint embeddings}

\paragraph{Unsupervised embeddings}

\paragraph{Supervised embeddings}
