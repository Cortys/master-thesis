...
\begin{equation}
	C_k(G_i) \coloneqq \bigcup_{\mathclap{v_j \in \mathcal{V}_{G_i}}}\, {\left\{ \chi_{G_i, 1}^{(l)}(v_j) \right\}}_{l = 0}^L % chktex 21
...
\begin{figure}[ht]
	...
	\caption[Illustration of the correspondence between the \ac{wl} color $\chi_{G,1}^{(l)}(v)$ and the breadth-first subtree of depth $l$ rooted at $v$.]{
		Illustration of the correspondence between the \ac{wl} color $\chi_{G,1}^{(l)}(v)$ and the breadth-first subtree of depth $l$ rooted at $v$ (for $l = 2$ and $v = \texttt{\textcolor{t_blue}{\circled{1}}}$). % chktex 25
...
\begin{align}
	{\varphi_{\text{SP}}(G)}_i \coloneqq \left| \left\{ (v, u) \in \mathcal{V}_G^2 \,|\, {\chi_{G,1}^{(l_i)}(v) = a_i} \land {\chi_{G,1}^{(l_i)}(u) = b_i} \land {d_{\text{SP}}(v, u) = d_i} \right\} \right| \text{,} % chktex 21
\end{align}
...
\begin{enumerate}[label=\textbf{\arabic*.}]
	\item \textbf{$k$-multisets:}
		...
		\begin{align}
			\chi_{G,k}^{(l+1)}(s) \coloneqq h\left(\chi_{G,k}^{(l)}(s), \ldblbrace \{ \chi_{G,k}^{(l)}(s \setminus \{v\} \cup \{u\})\, |\, v \in s \}\, |\, u \in \mathcal{V}_G \rdblbrace\right) \text{.} \label{eq:related:wlk-set-refine} % chktex 21
		\end{align}
...
Instead, by interpreting $\hat{g}_{\theta}$ as a matrix polynomial, one only has to compute the powers $L^1, \dots, L^{K-1}$ and the largest eigenvalue $\max{\lambda_G}$ which is generally much cheaper than computing the full spectrum.
...
By fixing $K = 2$, by using a single filter parameter $\theta \in \mathbb{R}$ and by assuming that $\max{\lambda_G^{\text{sym}}} \approx 2$ (see \cref{defn:related:laplacian-sym}, \cpageref{defn:related:laplacian-sym}) the convolution operation is reduced to
\begin{align}
...
To see why this is the case, we rewrite \cref{eq:related:gcn-layer} and compare it with the 1-\acs{wl} color refinement strategy (see \cref{defn:related:wl1-refine}, \cpageref{defn:related:wl1-refine}):
\begin{align*}
	\text{\ac{gcn}:}\quad&& Z^{(i)}[v] &= \sigma\left(\left( {c(v)}^2 Z^{(i-1)}[v] + \smashoperator[lr]{\sum_{u \in \Gamma_G(v)}} c(v) c(u) Z^{(i-1)}[u] \right) \Theta^{(i)} \right) \\
	\text{1-\acs{wl}:}\quad&& \chi_{G,1}^{(i)}(v) &= h\left( \chi_{G,1}^{(i-1)}(v), \ldblbrace \chi_{G,1}^{(i-1)}(u)\, |\, u \in \Gamma_{G}(v) \rdblbrace\right)
\end{align*}
Here $c(v) \coloneqq {\left(\left| \Gamma_G(v) \right| + 1\right)}^{-\frac{1}{2}}$ are normalization factors introduced by $\tilde{D}_G^{-\frac{1}{2}}$.
The comparison shows that \ac{gcn} convolutions can be understood as iterative refinements of continuous ``color vectors'' $Z^{(i)}[v] \in \mathbb{R}^{d^{(i)}}$.
Then the continuous analogue to the injective 1-\acs{wl} hash function $h: \mathcal{C}^{*} \to \mathcal{C}$ can be defined as $h_{\text{\acs{gcn}}}(z_0, z_1, \dots, z_m) = \sigma(\sum_{j=0}^m c(v_0) c(v_j) z_j \Theta^{(i)})$ which computes a single color vector from multiple color vectors $z_j$.
...
\begin{align}
	Z^{(i)}[v] \coloneqq \text{\acs{mlp}}^{(i)}\left( Z^{(i-1)}[v] + \smashoperator[lr]{\sum_{u \in \Gamma_G(v)}} Z^{(i-1)}[u] \right)
\end{align}
By leaving out the normalization factors $c(v)$ and by using a \ac{mlp} instead of the single fully-connected layer $\sigma \circ \Theta^{(i)}$ the resulting hashing function $h_{\text{\acs{gin}}}(z_1, \dots, z_m) = \text{\acs{mlp}}^{(i)}(\sum_{j=1}^{m} z_j)$ becomes injective\footnote{
...
The majority of \ac{gnn} methods uses a neighborhood aggregation scheme which limits their discriminative and, more importantly, computational power to that of 1-\acs{wl} (see \cref{defn:related:wl-count-detect}, \cpageref{defn:related:wl-count-detect}).
...
\begin{align}
	Z^{(i)}[s] \coloneqq \sigma\left( {Z^{(i-1)}[s]} W^{(i)} + \smashoperator[lr]{\sum_{v \in s, u \in \Gamma_G(v)}} {Z^{(i-1)}[s \setminus \{ v \} \cup \{ u \}]} W_{\Gamma}^{(i)} \right) \text{.} \label{eq:related:kgnn-layer}
\end{align}
It convolves the feature vectors of $k$-multisets $s \subseteq \mathcal{V}_G$ instead of single vertices $v \in \mathcal{V}_G$.
Apart from this fundamental difference to 1-\acs{wl} bounded \acp{gnn}, $k$-\acp{gnn} additionally use two learnable parameter matrices $W^{(i)}, W_{\Gamma}^{(i)} \in \mathbb{R}^{d^{(i-1)} \times d^{(i)}}$ instead of the single $\Theta^{(i)}$ used in \acp{gcn}.
This separation of parameter matrix effectively fulfills the same purpose as the \ac{mlp} in \ac{gin}; it makes the implicitly defined $k$-\acs{gnn} hash function injective.
This can be seen by realizing that a stack of $k$-\acs{gnn} convolution layers with $W_{\Gamma}^{(i)} = 0$ for all but the first layer simulates a \ac{mlp}.

As a final remark, note that the concept of ``neighborhood'' used in \cref{eq:related:kgnn-layer} is slightly different from that used by the $k$-\acs{wl} algorithm (see \cref{defn:related:wlk-refine}, \cpageref{defn:related:wlk-refine}):
$k$-\acs{gnn} considers a single $k$-multiset as a neighbor while $k$-\acs{wl} calls a $k$-set of $k$-multisets a neighbor.
