...
The \acf{wl} algorithm~\cite{Weisfeiler1968}\cite{Cai1992} characterizes a graph $G = (\mathcal{V}, \mathcal{E})$ by assigning discrete labels $c \in \mathcal{C}$, called \textit{colors}, to vertex $k$-tuples $(v_1, \dots, v_k) \in \mathcal{V}^k$, where $k \in \mathbb{N}$ is the freely choosable \textit{\ac{wl}-dimension}.
...
The final \ac{wl} coloring $\bar{\chi}_{G, k}$ that is obtained after convergence can be used to quickly check whether two graphs $G, H$ are not isomorphic ($G \not\simeq H$).
\begin{defn}\label{defn:related:wl-distinguishable}
	Two graphs $G$ and $H$ are are $k$-\acs{wl} \textit{distinguishable} ($G \mathrel{{\not\simeq}_k} H$) iff.\ there exists a color $c \in \mathcal{C}$ s.t.\ $\left|\left\{ a \in \mathcal{V}_G^k\, |\, \bar{\chi}_{G, k}(a) = c \right\}\right|\neq \left|\left\{ b \in \mathcal{V}_H^k\, |\, \bar{\chi}_{H, k}(b) = c \right\}\right|$. % chktex 21
\end{defn}
It is easy to show that $G \mathrel{{\not\simeq}_k} H \implies G \not\simeq H$.
...
\begin{figure}[ht]
	...
	\caption[Example 1-WL color refinement steps.]{
		...
		Each vertex is labeled with its current color and has the aggregated neighbors, as in \cref{defn:related:wl1-refine}, written next to it.
...
\begin{defn}\label{defn:related:wlk-refine}
	$\begin{aligned}[t]
		...
		\text{with } v = &\ (v_1, \dots, v_k) \in \mathcal{V}^k, v[u/j] \coloneqq (v_1, \dots, v_{j-1}, u, v_{j+1}, \dots, v_k) \text{.}
	\end{aligned}$
\end{defn}
...
\end{hproof}
\begin{fact}[see~\citet{Immerman1990}]\label{fact:related:wl-upper}
	For all $k$ there are non-isomorphic graphs $G \not\simeq H$ of size $\mathcal{O}(k)$ with $G \mathrel{\simeq_k} H$.
\end{fact}
\begin{figure}[ht]
	\centering
...
\begin{cor}\label{cor:related:wl-monotonous}
	The discriminative power of $k$-\acs{wl} grows monotonously with $k$ and never converges, i.e.\ for all $k \in \mathbb{N}$ there is an $l \in \mathbb{N}$ s.t.\ the set of $k$-\ac{wl} distinguishable graphs is a true subset of the $(k+l)$-\ac{wl} distinguishable graphs.
\end{cor}
\begin{hproof}
	Note that $k$-\acs{wl} trivially solves the graph isomorphism problem for all graphs of size $\leq k$ via the initial coloring (see~\cref{eq:related:wlk-init}).
	Using \cref{lem:related:wl-lower} and \cref{fact:related:wl-upper} the corollary directly follows.
\end{hproof}
...
\begin{equation}
	C_k(G_i) \coloneqq \bigcup_{\mathclap{v_j \in \mathcal{V}_i}}\, {\left\{ {\chi_{G_i, 1}^l(v_j) \right\}}_{l = 0}^k % chktex 21
