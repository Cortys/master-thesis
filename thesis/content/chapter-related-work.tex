...
\begin{lem}
	$G \mathrel{\not\simeq_k} H \implies G \mathrel{\not\simeq_{k+1}} H$ (the \acs{wl} power is never lower for higher $k$).
\end{lem}
\begin{hproof}
...
\end{figure}
\Cref{fig:related:wl2-problem-solution} illustrates the higher discriminative power of higher \acs{wl}-dimensions;
it shows how 2-WL is able to distinguish the two 1-WL indistinguishable graphs from \cref{fig:related:wl1-problem}.
...
\begin{equation}
	C_k(G_i) \coloneqq \bigcup_{\mathclap{v_j \in \mathcal{V}_i}}\, {\left\{ {\textrm{WL}_l(G_i)}_j \right\}}_{l = 0}^k % chktex 21
	\text{ with } \textrm{WL}_l(G) \coloneqq \text{$l$ iter.\ \acs{wl}-1 coloring of $G$.}
\end{equation}
