...
\begin{defn}\label{defn:ltag:substruct-embedding}
	...
	there are decomposition functions $\psi_{\varphi, i}: \mathcal{G} \to \mathcal{P}(\mathcal{G})$ and functions $\gamma_{\varphi, i}: \mathcal{G} \to (\mathcal{G} \to \mathbb{N}_0)$ for all embedding components $i \in [d]$ s.t.\ %
	$\forall G \in \mathcal{G}, i \in [d]: \varphi(G)[i] = \sum_{c \in \psi_{\varphi, i}(G)} \gamma_{\varphi, i}(G)(c)$ where $\gamma_{\varphi, i}(G): \mathcal{G} \to \mathbb{N}_0$ is the multiplicity function of a multiset of the constituents $\psi_{\varphi, i}(G)$.
...
\begin{thm}\label{thm:ltag:svm-ltag-formulation}
	A binary \ac{svm} graph classifier $h$ that applies a \ac{sce} $\varphi$ to its inputs has an \ac{lta} formulation that uses the underlying decomposition function $\psi_{\varphi}$.
\end{thm}
\begin{proof}
	...
	\begin{align*}
		...
		&= \sgn\left( {\sum_{c_t \in \psi_{\varphi}(G)} \underbrace{\sum_{i = 1}^d \gamma_{\varphi, i}(G)(c_t) \beta_i}_{z_t}} + b \right)
...
\begin{align}
	y_t \coloneqq \sgn z_t
	 &= \sgn\left( \sum_{j=1}^N \underbrace{\left( \alpha_j \smashoperator[l]{\sum_{c_k \in \psi_{\varphi}(G_j)}} \sum_{i=1}^d \gamma_{\varphi,i}(G)(c_t) \gamma_{\varphi,i}(G_j)(c_k) \right)}_{w_j} y_j \right) \\
...
\begin{enumerate}[label=\textbf{\arabic*.}]
	\item \textbf{Fingerprint embeddings:}
		...
		Such a fingerprint embedding naturally is an \ac{sce} with the decomposition functions $\psi_{\text{FP}, i}(G) \coloneqq \{ G[\set(s)]\, |\, s \in \mathcal{V}_G^{*} \land G[s] \equiv S_i \}$ and the multiplicity functions $\gamma_{\text{FP}, i}(G)(c) \coloneqq \mathbbm{1}[c \in \psi_{\text{FP}, i}(G)]$. % chktex 21
		...
		\badgebox[t_green]{\cmark\ LTA}
	\item \textbf{\texttt{graph2vec}:}
		The skip-gram inspired \texttt{graph2vec} embedding produces vectors $\text{FP}(G) \in \mathbb{R}^d$ whose individual components do not have a clear interpretation.
		Since the component values can be arbitrary reals, this approach is not an \ac{sce} which in turn implies that it does not have an \ac{lta} formulation as in \cref{thm:ltag:svm-ltag-formulation}.
		\badgebox[t_red]{\xmark\ non-LTA}
	\item \textbf{\ac{wl} subtree kernel:}
		The individual components of the \ac{wl} subtree embedding represent the number of occurrences of a color $c_i \in \mathcal{C}$, i.e.\ $\varphi_{\text{ST}}(G)[i] \coloneqq \mathit{dist}_{\chi_{G,1}^{(t)}}(c_i)$ as defined in \cfullref[ on ]{eq:related:wl-subtree-kernel}.
		As illustrated in \cfullref[ on ]{fig:related:wl-subtree}, each occurrence of a color $c_i$ in the \ac{wl} coloring $\chi_{G,1}^{(t)}$ corresponds to the occurrence of a breadth-first subtree of depth $t$.
		\badgebox[t_green]{\cmark\ LTA-like}
