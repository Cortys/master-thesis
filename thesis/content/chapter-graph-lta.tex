...
\begin{enumerate}[label=\textbf{\arabic*.}]
	...
	\item \textbf{Aggregation:}
		...
		\begin{defn}\label{defn:ltag:weighted-agg}
			...
			\begin{align*}
				\text{idempotency: } & \exists c \geq 0: \forall y \in \mathcal{Y}, w \in \mathbb{R}_{\geq 0}^n\text{ s.t. }{\max w} \geq c: \mathcal{A}({\ldblbrace (y, w_i) \rdblbrace}_{i=1}^n) = y \\
		...
		One example for such a weighted aggregator is the \textit{weighted mean} function $\wmean({\ldblbrace (y_i, w_i) \rdblbrace}_{i=1}^n) \coloneqq \sum_{i=1}^n w_i y_i$ which requires $\sum w_i = 1$ and $w_i \in [0, 1]$.
		Alternatively an unweighted aggregation function like $\min$, $\max$ or \ac{owa}\footnote{
			Even though \ac{owa} uses weights, it does not get those weights as part of the input and is therefore considered to be unweighted here.
...
\begin{defn}\label{defn:ltag:substruct-embedding}
	...
	$\forall i \in [d]: \exists S_i \in \mathcal{G}_{\mathit{ord}}, \gamma_i \in \mathbb{R}: \forall G \in \mathcal{G}: {{\varphi(G)}[i] = \mathit{contains}_{S_i}(G)} \lor {{\varphi(G)}[i] = \gamma_i \mathit{count}_{S_i}(G)}$,
	with $\mathit{contains}$ and $\mathit{count}$ as in \cfullref[ on ]{defn:related:wl-count-detect}.
\end{defn}
...
\begin{proof}
	...
	{\allowdisplaybreaks\begin{align*}
		...
		&= \sgn\left( {\sum_{i=1}^{d} {\varphi(G)}[i] \underbrace{\left( \sum_{j = 1}^{N} \alpha_j y_j {\varphi(G_j)}[i] \right)}_{{} \eqqcolon z_i}} + b \right)
		\text{.}
	\end{align*}}
	...
	more precisely:
	\begin{itemize}
		\item If $\varphi(G)[i] = \gamma_i \mathit{count}_{S_i}(G) > 0$, let $C_i \coloneqq {\{ G[s]\, |\, s \in \mathcal{P}(\mathcal{V}_G) \land G[s] \simeq S_i \}}$ be the $\mathit{count}_{S_i}(G)$ subgraphs counted by the $i$-th dimension.
		\item If $\varphi(G)[i] = \mathit{contains}_{S_i}(G) = 1$, let $C_i \coloneqq {\{  \}}$.
	\end{itemize}
	A substructure component embedding vector can therefore be interpreted as an encoding of the graph decomposition \[
		\psi(G) \coloneqq \bigcup_{i=1}^{d}
		\text{.}
	\]
	\begin{align*}
		h(G) &= \sgn\left( {\sum_{i=1}^{d} {\varphi(G)}[i] \underbrace{|z_i|}_{{} \eqqcolon \nu_i} \underbrace{\sgn(z_i)}_{{} \eqqcolon y_i}} + b \right)
	\end{align*}
\end{proof}
