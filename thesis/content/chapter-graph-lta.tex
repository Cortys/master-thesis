	...
	\item \textbf{\ac{wl} subtree kernel:}\label{itm:ltag:wlst-lta-formulation}
		...
		\begin{figure}[t]
			...
			\caption[The constituents implied by the \ac{wl} subtree kernel embedding for an example graph.]{
				...
				Because $S_{1,1}$ and $S_{2,1}$ span the same constituent $c = c_{1,1} = c_{2,1} = G[\{ v_1, v_2, v_3 \}]$, its multiplicity is $\gamma_{\varphi, \colorlabel{t_red}{\tiny B}, 1}(c) = 2$; % chktex 25
		...
		To see why this is required, note that in \cref{fig:ltag:wl1-constituents} the $c_{1,1}$/$c_{2,1}$ constituent contains the color subtree $S_{\colorlabel{t_red}{\tiny B}}$ three times (rooted at $v_1$, $v_2$ and $v_3$) even though $G$ only contains it twice (rooted at $v_1$ and $v_2$). % chktex 25
...
\begin{figure}[t]
	\centering
	\includegraphics[width=0.73\linewidth]{gfx/graph-lta/gcnn-lta.pdf}
	\caption[Illustration of the \ac{lta} formulation for \acp{gcnn}.]{
		Illustration of the \ac{lta} formulation for \acp{gcnn}, as shown in \cref{thm:ltag:gcnn-ltag-formulation}.
		Constituent weights are left out for simplicity.
	}\label{fig:ltag:gcnn-lta}
\end{figure}
