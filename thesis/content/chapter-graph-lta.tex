...
\begin{enumerate}[label=\textbf{\arabic*.}]
	\item \textbf{Convolution:}
		...
		The convolution layers produce a convolved feature matrix $Z \in \mathbb{R}^{n \times d^{(T)}}$ where each row is a convolved feature vector $z_i \in \mathbb{R}^{d^{(\text{pool})}}$.
	\item \textbf{Pooling:}
...
\begin{defn}\label{defn:ltag:assoc-aggregation-pooling}
	A pooling layer $\mathit{Pool}: \mathbb{R}^{n \times d^{(T)}} \to \mathbb{R}^{d^{(\text{pool})}}$ is called an \textit{associative weighted aggregation layer} iff.
	\begin{enumerate*}
...
\begin{thm}\label{thm:ltag:gcnn-ltag-formulation}
	A \ac{gcnn} of the form $\mathit{Pool}(\mathit{Conv}(G))$ has a nontrivial \ac{lta} formulation if $\mathit{Conv}$ is a stack of neighborhood convolution layers and $\mathit{Pool}$ is an associative weighted aggregation layer.
\end{thm}
\begin{proof}
	...
	a formulation for vertex $k$-multiset neighborhood convolutions can be obtained analogously which concludes the proof\footnote{
		The \ac{lta} formulation in this proof assumes that the \ac{gcnn}'s output is translated into the target score space via $\tau_{2}: \mathbb{R}^{d} \to \mathcal{Y}$.
		All this means is that an \acs{lta}-like \ac{gcnn} must produce outputs $y \in \mathcal{Y}$;
		for example if a \ac{gcnn} returns one-hot vector encodings of discrete classes those must be decoded.
	}.
\end{proof}
...
\begin{enumerate}[label={\textbf{\arabic*.}}]
	...
	\item \textbf{SortPooling \& \ac{sagpool}:}
		...
		\begin{enumerate*}[label={\arabic*.}]
			\item SortPooling~\cite{Zhang2018} works by aggregating the top-$p$ convolved vectors $z_i \in \mathbb{R}^{d^{(T)}}$ based on a lexicographic ordering,
			\item \ac{sagpool}~\cite{Lee2019} also aggregates the top-$p$ vectors $z_i$ but determines their order based on attention weights which are encoded as one component of each $z_i$
		\end{enumerate*}.
		...
		This is shown by the fact that there is no $\rho: \mathbb{R}^{* \times d^{(T)}} \to \mathbb{R}^{ d^{(T)}}$ s.t.\ for all vectors $z_1 < z_2$ and and aggregation functions $\mathcal{A}$ it holds that%
		\begin{alignat*}{2}
			...
			\Leftrightarrow&& \mathcal{A}(\ldblbrace z_2, z_2 \rdblbrace) &= \mathcal{A}(\ldblbrace z_1, \rho(\ldblbrace z_2, z_2 \rdblbrace) \rdblbrace)\quad\lightning
		\end{alignat*}
...
Since neither SortPooling nor \ac{sagpool} are compatible with \ac{lta}, we propose an associative weighted aggregation layer based on \ac{sagpool} which uses a softmax-based attention filter instead of a top-$p$ filter.
\begin{defn}
	We define \ac{sampool} as
	\[\mathit{SAM}({\ldblbrace (y_i, w_i) \rdblbrace}_{i=1}^n) \coloneqq\frac{1}{\sum_{i=1}^n e^{w_i}} \sum_{i=1}^n e^{w_i} y_i \text{.}\]
\end{defn}
...
To see why this is the case, the associativity and homomorphicity to a weighted aggregation function $\mathcal{A}_{\text{SAM}}$ must be shown.
The associativity of $\mathit{SAM}$ follows directly from its commutativity and the fact that
\begin{gather*}
	\mathit{SAM}({\ldblbrace (y_i, w_i) \rdblbrace}_{i=1}^n) = \frac{1}{e^{w'} + \sum_{i=j+1}^n e^{w_i}} \left( e^{w'} \underbrace{\mathit{SAM}({\ldblbrace (y_i, w_i) \rdblbrace}_{i=1}^j)}_{y'} + \smashoperator{\sum_{i=j+1}^n} e^{w_i} y_i \right) \\
	\text{for all } j \in [n] \text{ with } w' \coloneqq \ln\sum_{i=1}^{j} e^{w_i} \text{, i.e.\ } \rho_{\mathit{SAM}}({\ldblbrace (y_i, w_i) \rdblbrace}_{i=1}^j) = (y', w')\text{.}
\end{gather*}
The homomorphicity of $\mathit{SAM}$ to a weighted aggregation function $\mathcal{A}_{\text{SAM}}$ follows by choosing $\mathcal{A}_{\text{SAM}}({\ldblbrace (y_i, w_i) \rdblbrace}_{i=1}^n) \coloneqq {\left(\sum_{i=1}^n w_i\right)}^{-1} \sum_{i=1}^n w_i y_i$ and the trivial homomorphism $\tau_1(y, w) \coloneqq (y, e^w)$, $\tau_2(y) \coloneqq y$ between the algebras of $\mathit{SAM}$ and $\mathcal{A}_{\text{SAM}}$.
...
The only requirement for this formulation to exist is that an associative pooling layer like $\mean$, $\min$, $\max$ or $\mathit{SAM}$ is used to combine the convolved feature vectors into the final output.
