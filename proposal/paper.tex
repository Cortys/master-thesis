...
The assumption in LTA problems is that for all constituents $c_i$ a local score $y_i \in \mathcal{Y}$ is either given or computable.
The set of those local scores should be indicative of the overall score $y \in \mathcal{Y}$ of the entire composition $\bm{c}$.
The goal of LTA is to learn a variadic aggregation function $A: \mathcal{Y}^{*} \to \mathcal{Y}$ that estimates such composite scores, i.e.\@ $\hat{y} = A(y_1, \dots, y_n)$ for a composition with $n$ constituents.
